...
const actions = {
  ...
  async fetchCourseEnrollments({ commit }, courseId) {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:3000/api/courses/${courseId}/enrollments`, {
  ...
  async fetchAvailableStudents({ commit }, courseId) {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:3000/api/courses/${courseId}/available-students`, {
        headers: { Authorization: `Bearer ${token}` }
      });
